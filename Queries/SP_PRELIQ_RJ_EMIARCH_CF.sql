CREATE OR REPLACE PROCEDURE "SP_PRELIQ_RJ_EMIARCH_CF" (
        ...
        (select 1 num, rpad('1' ||lpad(TO_CHAR(CIA_RUT_CIA_ID), 9,0 )||
                ...
                (select lpad(sum( ben ), 6,0)|| lpad(sum(nvl(monto,0)), 15,0) from 
                        (select lpad(count(distinct dsg_ben ), 6,0) ben, 
                         ...
                         from pensiones.dsgdetrv d,pensiones.retjud R, pensiones.persnat P, 
                              pensiones.liqrv g, pensiones.domicilios c
                         where dsg_per= 202404
                               ...
                               and ((rj_vig = 1) or (rj_vig = 2 and rj_mes_fin = 202404 ))
                        and RJ_BCO_NN = '012'
