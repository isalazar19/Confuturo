...
SELECT * FROM rentarsv.ris_treg_1 WHERE r1_cia_id =2 AND fec_cie = 20230331;

SELECT * FROM rentarsv.ris_treg_2 WHERE r2_cia_id =2 AND fec_cie = 20230331;  

SELECT * FROM rentarsv.ris_treg_3 WHERE r3_cia_id =2 AND fec_cie = 20230331;

SELECT * FROM rentarsv.ris_treg_4 WHERE r4_cia_id =2 AND fec_cie = 20230331;
...
SELECT   NVL (TO_CHAR (LPAD (svsb_poliza_id, 10, '0')), '0000000000')
     ...
     , LPAD (NVL (svsb_rt_base_total_nn / 0.01, '0'), 7, 0) Z
     , LPAD (NVL (svsb_rt_base_2009_total / 0.01, '0'), 7, 0)
     , LPAD (NVL (svsb_rt_fin_2004_85_total / 0.01, '0'), 7, 0) AB
...
select svsb_rt_base_total_nn,svsb_rt_fin_2004_85_total
from rentarsv.svs_beneficiario
WHERE    svsb_periodo_id = 202303
      AND svsb_poliza_id = 660613
