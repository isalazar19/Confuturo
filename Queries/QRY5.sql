        ...
        case when mod_tipo_modif = 57 and mod_dig_nue <> 0 then 
         ...
         end_nro_endoso,
          case when mod_tipo_modif = 181
                then 'INGRESO DE NUEVO BENEFICIARIO PÓLIZA Nº '||to_char(end_poliza) 
             when mod_tipo_modif = 55
                then 'CAMBIO RELACIÓN DE BENEFICIARIO PÓLIZA Nº '||to_char(end_poliza)  
         when mod_tipo_modif = 75 or mod_tipo_modif = 58 
                then 'REMOCIÓN DE BENEFICIARIO PÓLIZA Nº '||to_char(end_poliza)  
         when mod_tipo_modif = 57 
                then 'MODIFICA FECHA DE NACIMIENTO CAUSANTE/BENEFICIARIO PÓLIZA Nº '||to_char(end_poliza)  
         when mod_tipo_modif = 59
                then 'MODIFICA SEXO CAUSANTE/BENEFICIARIO PÓLIZA Nº '||to_char(end_poliza) 
         when mod_tipo_modif = 60
                then 'MODIFICA CONDICIÓN INVALIDEZ PÓLIZA Nº '||to_char(end_poliza)  
      END tipo_endoso,
      (select decode(ben_invalido, 1,'Inválido total', 3, 'Inválido parcial', 'No inválido') 
         ...
         nvl((decode(mod_cantidad_nue,0, end_prima_nue, mod_cantidad_nue) - decode(mod_cantidad_ant, 0, end_prima_ant, mod_cantidad_ant) ),0) diferencia,
          ...
          case when mod_tipo_modif = 181
		       ...
		       then 'REMOCIÓN DE BENEFICIARIO PÓLIZA Nº '||to_char(end_poliza) 
                ...
                then 'MODIFICA FECHA DE NACIMIENTO CAUSANTE/BENEFICIARIO PÓLIZA Nº '||to_char(end_poliza)  
				 when mod_tipo_modif = 59
                then 'MODIFICA SEXO CAUSANTE/BENEFICIARIO PÓLIZA Nº ' 
				 when mod_tipo_modif = 60
                then 'MODIFICA CONDICIÓN INVALIDEZ PÓLIZA Nº '||to_char(end_poliza)  
			END tipo_endoso,
      (select decode(ben_invalido, 1,'Inválido total', 3, 'Inválido parcial', 'No inválido') 
  ...
  SELECT end_asegurado,
       ...
       case
         when mod_tipo_modif = 181 then
          'INGRESO DE NUEVO BENEFICIARIO PÓLIZA Nº ' || to_char(end_poliza)
		 when mod_tipo_modif = 55 and bnf_estado = 1  
		then 'REMOCIÓN DE BENEFICIARIO PÓLIZA Nº '||to_char(end_poliza)  
         when mod_tipo_modif = 55 then
          'REMOCIÓN DE BENEFICIARIO PÓLIZA Nº ' || to_char(end_poliza)
         when mod_tipo_modif = 75 or mod_tipo_modif = 58 then
          'REMOCIÓN DE BENEFICIARIO PÓLIZA Nº ' || to_char(end_poliza)
         when mod_tipo_modif = 57 then
          'MODIFICA FECHA DE NACIMIENTO CAUSANTE/BENEFICIARIO PÓLIZA Nº ' ||
          to_char(end_poliza)
         when mod_tipo_modif = 59 then
          'MODIFICA SEXO CAUSANTE/BENEFICIARIO PÓLIZA Nº '
         when mod_tipo_modif = 60 then
          'MODIFICA CONDICIÓN INVALIDEZ PÓLIZA Nº ' || to_char(end_poliza)
       END tipo_endoso,
