CREATE OR REPLACE PROCEDURE pensiones."SP_CT_GENARCH_CARTERA_EXCEL" ( p_periodo IN NUMBER,
                                               p_cod_error     out number,
                                               p_msj_error     out varchar2 )
IS
/*************************************************************************
Sistema           : PENSIONES
Subsistema        : Liquidacion de Pensiones - PGU
  ...
  --  run_id number;  /*  Variable para implementacion dbms_profiler*/
   ...
   CURSOR obt_ct_liqrv IS
           SELECT distinct
             ...
             ltrim(nvl(translate(lq.LQR_CTA_BCO, 'EK.-', ' '),'0')) LQR_CTA_BCO,
             lq.LQR_FRM_PAGO,
             lq.lqr_id_dom,
             d1.dsg_ben,
             lqr_id_recep ,
...
--            and lq.lqr_rep = d1.dsg_ben     -- se agrega nueva condicion
            and d1.dsg_tipo in (1)
...
/*  Dbms_profiler */
      ...
      begin
        delete PENSIONES.CT_PGU_DATOS_PAGO_PERIODO
        where pdp_periodo = ll_periodo;
      exception
              WHEN OTHERS THEN
                p_cod_error := 1;
                p_msj_error := ' Borrando ct_pgu_datos_pago_periodo  : ' || SQLCODE ||
                               ' -ERROR- ' || SQLERRM;
                return;
      end;
      commit;
   ...
   /*    insert into pensiones.tmp_proceso_log( tpl_accion,   tpl_fecha_log  )
       ...
       FOR c_obt_ct_mte IN obt_ct_mte LOOP
           ...
           select upper(rpad(F_LIMPIA_CARACTER(nvl(db.dom_direccion, ' ')),45)) ,
             ...
             to_char(lpad(nvl(F_IS_NUMBERO(f_limpia_string_2(db.dom_telef_2),9) ,0),9,'0')) ,
              ...
              case when db.dom_e_mai_cr LIKE '%@%' then
                   upper(rpad(REPLACE(REPLACE(REPLACE(REPLACE(translate(ltrim(rtrim(db.dom_e_mai_cr)), 'AEIOUaeiou???#', 'AEIOUaeiouNn N'),CHR(27),' '), CHR(0),NULL), CHR(9),NULL), CHR(13),NULL),45))
              else  '                                             ' end
          ...
          v_tot_desctos:= '00000000';
           ...
           Begin
             insert into PENSIONES.ct_pgu_datos_pago_periodo
            ...
            exception
              WHEN OTHERS THEN
                p_cod_error := 1;
                p_msj_error := ' insertando ct_pgu_datos_pago_periodo mte: ' || SQLCODE ||
                               ' -ERROR- ' || SQLERRM;
                return;
            end;
      END LOOP;
...
--tls
         ...
         if ll_frm_pago = 15 or ll_frm_pago = 10 then
           ...
           Begin
            ...
            exception
              WHEN OTHERS THEN
                p_cod_error := 1;
                p_msj_error := ' insertando ct_pgu_datos_pago_periodo : ' || SQLCODE ||
                               ' -ERROR- ' || SQLERRM;
                return;
            end;
  ...
  /*    insert into pensiones.tmp_proceso_log( tpl_accion,   tpl_fecha_log  )
       ...
       Begin
        ...
        Begin
            update ct_pgu_datos_pago_periodo
        ...
        exception
              WHEN OTHERS THEN
                p_cod_error := 1;
                p_msj_error := ' modificando  ct_pgu_datos_pago_periodo : ' || SQLCODE ||
                               ' -ERROR- ' || SQLERRM;
                return;
        end;
        commit;
